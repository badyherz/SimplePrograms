...
def get_month_display_name(month):
    ...
    return t(
        MONTH_TRANSLATION_KEYS[index]
    )

def get_display_months():
    
    return [
        get_month_display_name(month)
...
# Settings Button
# -------------------------
settings_button = tk.Menubutton(
    toolbar,
    text=f"⚙ {t('menu.settings')}",
    relief="raised"
)
...
settings_menu.add_command(
    label=t("menu.import_json"),
    command=lambda: import_data()
)
settings_menu.add_command(
...
for currency in CURRENCIES:
    currency_menu.add_command(
        label=currency,
        command=lambda c=currency: change_currency(c)
    )
...
settings_menu.add_command(
    label=t("menu.delete_all"),
    command=lambda: delete_all_data()
)
...
show_all_button = tk.Button(
    toolbar,
    text=t("button.show_all_months"),
    command=show_all_months
)
...
# -------------------------
current_month_button = tk.Button(
    toolbar,
    text=t("button.current_month"),
    command=show_current_month
)
current_month_button.pack(
    side="left",
    padx=(5,0)
)
...
statistics_button = tk.Button(
    control_frame,
    text=t("button.statistics"),
    command=lambda: open_statistics_window()
)
...
canvas, months_container = create_scrollable_frame(
    scroll_container
)
...
def create_month_cards():
    ...
    if selected_month.get() != t("label.all_months"):
        ...
        month = get_internal_month(
            selected_month.get()
            )

        card = tk.LabelFrame(
            months_container,
            text=f"{get_month_display_name(month)} {selected_year.get()}",
            padx=20,
            pady=20
        )
    ...
    for index, month in enumerate(MONTH_NAMES):
        ...
        card = tk.LabelFrame(
            months_container,
            text=get_month_display_name(month),
            padx=10,
            pady=10
        )
    ...
    year_total_frame = tk.LabelFrame(
        months_container,
        text=t("label.year_total"),
        padx=15,
        pady=10
    )


    year_total_frame.grid(
        row=3,
        column=0,
        columnspan=4,
        sticky="w",
    ...
    energy_total = calculate_year_total(
        selected_year.get(),
        "Electricity"
    )


    grocery_total = calculate_year_total(
        selected_year.get(),
        "Groceries"
    )
    ...
    tk.Label(
        energy_frame,
        text=f"{t('label.electricity')}: "
    ).pack(
        side="left"
    )


    year_energy_total_label = tk.Label(
        energy_frame,
        text=f"{energy_total:.2f} {selected_currency.get()}",
        font=("Arial", 10, "bold")
    )

    year_energy_total_label.pack(
        side="left"
    )

    #Groceries
    grocery_frame = tk.Frame(
        year_total_frame
    )
    ...
    tk.Label(
        grocery_frame,
        text=f"{t('label.groceries')}: "
    ).pack(
        side="left"
    )


    year_grocery_total_label = tk.Label(
        grocery_frame,
        text=f"{grocery_total:.2f} {selected_currency.get()}",
        font=("Arial", 10, "bold")
    )

    year_grocery_total_label.pack(
        side="left"
    )
...
def create_trend_box(parent, month):
    ...
    if month_index > 0:
        ...
        tk.Label(
            box,
            text=t("message.no_previous_month")
        ).pack(
            anchor="w"
        )
    ...
    tk.Label(
        box,
        text=f"{t("trend.same_month_last_year")}:",
        font=("Arial", 9, "bold")
    ).pack(
        anchor="w",
        pady=(10,0)
...
def update_year_total_display():
    ...
    energy = calculate_year_total(
        year,
        "Electricity"
    )


    groceries = calculate_year_total(
        year,
        "Groceries"
    )


    year_energy_total_label.config(
        text=f"{t("label.electricity")}: {energy:.2f} {selected_currency.get()}"
    )


    year_grocery_total_label.config(
        text=f"{t("label.groceries")}: {groceries:.2f} {selected_currency.get()}"
)
...
def export_statistics_charts():
    ...
    folder,
    f"{selected_year.get()}_monthly_expenses.png"
    )
    ...
    folder,
    f"{current_year-9}-{current_year}_yearly_average_trend.png"
    )


    month_chart_figure.savefig(
        month_path,
        dpi=300,
        bbox_inches="tight"
    )
...
def delete_all_data():
    ...
    second = messagebox.askyesno(
        t("dialog.warning_final"),
        f"{t("message.delete_warning2")} \n{t("dialog.confirm")}?"
    )
...
def calculate_average(year, category):
    ...
    for month in MONTH_NAMES:
        ...
        total += data["years"][year][month].get(
            category,
            0
        )
...
def open_statistics_window():
    ...
    # Scrollable area
    # -------------------------
    stats_canvas, frame = create_scrollable_frame(
    statistics_window
    )

    #Mousewheel
    def _mousewheel(event):
        stats_canvas.yview_scroll(
            int(-1 * (event.delta / 120)),
            "units"
        )
    ...
    tk.Label(
        frame,
        text=f"{t("statistics.title")} {year}",
        font=("Arial", 14, "bold")
    ).pack(
        anchor="w",
        pady=10,
    ...
    # Averages
    # -------------------------
    average_box = tk.LabelFrame(
        frame,
        text=t("statistics.average"),
        padx=10,
        pady=10
    )
    ...
    for category in ["Electricity", "Groceries"]:
        ...
        tk.Label(
            row,
            text=f"{avg:.2f} {selected_currency.get()}",
            font=("Arial", 10, "bold")
        ).pack(
            side="left"
        )
    ...
    # Previous 3 years
    # -------------------------
    comparison_box = tk.LabelFrame(
        frame,
        text=t("statistics.previous_years"),
        padx=10,
        pady=10
    )
    ...
    for back in range(1, 4):
        ...
        start=1
            ...
            difference, percent = get_year_difference(
                year,
                category,
                back
            )
            ...
            tk.Label(
                value_frame,
                text=f"{difference:+.2f} {selected_currency.get()} ({percent:+.1f}%)"
            ).pack(
                side="left"
            )
    ...
    frame,
    text=t("button.export_charts"),
    command=export_statistics_charts
    )
...
def get_monthly_values(year, category):
    ...
    return [
        data["years"][str(year)][month].get(category, 0)
        for month in MONTH_NAMES
    ]
...
def get_average_history(current_year, category):
    ...
    for year in range(current_year - 9, current_year + 1):
        ...
        averages.append(
            calculate_average(year, category)
        )
...
def create_year_trend_chart(parent, year):
    ...
    years, energy = get_average_history(
        year,
        "Electricity"
    )

    _, groceries = get_average_history(
        year,
        "Groceries"
    )
